...
Once write tools are available in the session, the same confirmation rules from `.raisely/raisely-guardrails.md` apply: summarize the change, confirm the environment (TEST vs LIVE), and never delete or unpublish without double confirmation.

## Write / destructive (CLI)

These change remote state. Always summarize first and follow `.raisely/raisely-guardrails.md`.
...
| Any `delete`, `archive`, or `unpublish` flag | Destructive                                                                       | Double confirmation per `.raisely/raisely-guardrails.md`        |
...
1. Run the `raisely-context` skill to bootstrap project context.
2. Run the `raisely-ensure-cli` skill.
3. `raisely list --json` -> identify the UUID (skip if `.raisely.json` already matches).
4. `raisely init --uuid <uuid>` if not already pulled.
5. Start `raisely local` as a background task (see `raisely-start-local` skill).
6. Edit files locally. Validate JSON / components.
7. Verify in the local preview (screenshot).
8. Summarize the diff for the user. Wait for approval.
9. `raisely deploy --force` only after explicit approval.
