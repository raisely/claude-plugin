...
Use `AskUserQuestion` for every question below. Fire the first batch in a single call, then use additional `AskUserQuestion` calls to resolve follow-ups. Never ask for raw text when a structured choice is possible.
...
**Batch 2 — slug confirmation** (separate `AskUserQuestion` call, after deriving the slug from the campaign name)
...
If the user picks "Generate suggestions based on my campaign" for hero copy, generate 2–3 headline and subheadline options and ask them to pick one via `AskUserQuestion` before proceeding.
...
Before running `raisely init`, confirm the target directory. Propose `~/Raisely/campaigns/<slug>` as the default (using the campaign slug from Step 2), then use `AskUserQuestion` with these options:
...
- **CTA button text**: search all pulled page JSON files for blocks with `"type": "RaiselyDonationButton"`. If any exist, use `AskUserQuestion` to ask the user what the button should say ("Donate now" / "Give today" / "Support us" / "Make a difference" / Other), then set the `text` field on every occurrence. If no such blocks exist, skip this step entirely — do not ask about CTA text.
